...
#学習させるデータに閾値を設定しデータを除去しモデルとの精度を比較するためのテストプログラム
...
def getdata(filename): #データ取得関数
    ...
    for i in range(csv_row): #csvファイルの列数分
        ...
        data_norm = data/max(data) #正規化

        shift_count = int(math.floor((len(data_norm) - input_data)/shift_number) + 1)
        for k in range(shift_count):
            list_data.append(data_norm[k*shift_number:k*shift_number+input_data])
...
model.save(MODEL_DIR + '/model_3ca_test.hdf5')  #学習モデルの保存
